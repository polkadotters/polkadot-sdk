...
impl<'a> sp_wasm_interface::Virtualization for HostContext<'a> {
	...
	fn execute(
		...
		let mut virt = match self.host_state_mut().virt_instances.remove(&instance_id) {
			Some(VirtOrMem::Instance(virt)) => virt,
			Some(VirtOrMem::Memory(_)) =>
				Err("it is illegal to call execute while already executing")?,
		...
		if destroy {
			...
			self.host_state_mut()
				.virt_instances
				.insert(instance_id, VirtOrMem::Instance(virt));
		}
